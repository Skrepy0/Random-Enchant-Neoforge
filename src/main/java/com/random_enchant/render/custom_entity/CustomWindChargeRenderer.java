...
public class CustomWindChargeRenderer extends EntityRenderer<CustomWindChargeEntity> {
    ...
    public void render(CustomWindChargeEntity entity, float entityYaw, float partialTick, PoseStack poseStack,
                       MultiBufferSource bufferSource, int packedLight) {
        if (entity.tickCount >= 2 || !(this.entityRenderDispatcher.camera.getEntity().distanceToSqr(entity) <
                                       (double) MIN_CAMERA_DISTANCE_SQUARED)) {
            float f = (float) entity.tickCount + partialTick;

            poseStack.pushPose();
            //            NeoMafishMod.LOGGER.info("x="+
            //            CustomWindChargeS2CPacket.CustomWindChargeData.getRadius(entity.getUUID()));
            ...
            VertexConsumer vertexconsumer =
                    bufferSource.getBuffer(RenderType.breezeWind(TEXTURE_LOCATION, this.xOffset(f) % 1.0F, 0.0F));
    ...
    protected float xOffset(float tickCount) { return tickCount * 0.03F; }
}
