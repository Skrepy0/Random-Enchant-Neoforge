...
public class CustomWindChargeEntity extends AbstractWindCharge {
    ...
    public CustomWindChargeEntity(Player player, Level level, double x, double y, double z) {
        super(com.random_enchant.entity.ModEntities.CUSTOM_WIND_CHARGE.get(), level, player, x, y, z);
    }
    ...
    public void tick() {
        ...
        if (radius == 1) {
            ...
            this.setDeltaMovement(
                    this.getDeltaMovement().multiply(decelerationFactor, decelerationFactor, decelerationFactor));
        }
        if (radius > 4) {
            // 定义一个半径为 `radius` 的范围，用于检测实体
            double attractionRadius = 0.3125 * radius + 1.5; // 吸引半径，可以根据需要调整

            AABB boundingBox = new AABB(this.getX() - attractionRadius, this.getY() - attractionRadius,
                                        this.getZ() - attractionRadius, this.getX() + attractionRadius,
                                        this.getY() + attractionRadius, this.getZ() + attractionRadius);
            ...
            for (Entity entity: nearbyEntities) {
                ...
                // 计算从实体到中心的方向向量
                Vec3 toCenter =
                        new Vec3(this.getX() - entity.getX(), this.getY() - entity.getY(), this.getZ() - entity.getZ());
                ...
                // 计算最终的速度矢量
                Vec3 finalVelocity =
                        currentVelocity.add(attractionVelocity).add(tangentialVelocity).add(gravityCounterVelocity);
                ...
                if (finalVelocity.length() > maxSpeed) {
                    // 将速度缩放到最大速度
                    RandomEnchant.LOGGER.info("finalVelocity=" + finalVelocity);
    ...
    protected void onHitEntity(EntityHitResult result) {
        ...
        if (entity instanceof CustomWindChargeEntity) { // 如果是自己
            ...
            PacketDistributor.sendToAllPlayers(
                    new CustomWindChargeS2CPacket(entity.getUUID(), ((CustomWindChargeEntity) entity).radius));
            this.discard();
        } else {
