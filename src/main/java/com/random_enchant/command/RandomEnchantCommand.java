...
public class RandomEnchantCommand {
    ...
    public static void register(RegisterCommandsEvent event) {
        event.getDispatcher().register(Commands.literal("random_enchant").then(Commands.literal("description").executes(context -> {
            Component modName = Component.literal("§6[§nRandom Enchant§r§6]§r").withStyle(style -> style.withUnderlined(true).withClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, "https://github.com/Skrepy0/Random-Enchant-Neoforge")).withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, Component.literal("点击打开§6Random Enchant§r 的Github仓库"))));
            ...
            return 1;
        })).then(Commands.literal("config").requires(source -> source.hasPermission(2)).then(Commands.literal("doRandomEnchant").then(Commands.argument("enabled", BoolArgumentType.bool()).executes(context -> {
            boolean enabled = BoolArgumentType.getBool(context, "enabled");
            Config.setRandomEnchant(enabled);
            Component message = enabled ? Component.translatable("command.random_enchant.randomEnchant.enable") : Component.translatable("command.random_enchant.randomEnchant.disable");

            RandomEnchant.LOGGER.info(message.getString());
            context.getSource().sendSuccess(() -> message, false);

            return 1;
        }))).then(Commands.literal("alwaysEnchantable").then(Commands.argument("enabled", BoolArgumentType.bool()).executes(context -> {
            ...
            context.getSource().sendSuccess(() -> message, false);
            return 1;
        }))).then(Commands.literal("infinityUndyingTotem").then(Commands.argument("enabled", BoolArgumentType.bool()).executes(context -> {
            ...
            context.getSource().sendSuccess(() -> message, false);
            return 1;
        }))).then(Commands.literal("infinityBlock").then(Commands.argument("enabled", BoolArgumentType.bool()).executes(context -> {
            ...
            context.getSource().sendSuccess(() -> message, false);
            return 1;
        }))).then(Commands.literal("infinityTnt").then(Commands.argument("enabled", BoolArgumentType.bool()).executes(context -> {
            ...
            context.getSource().sendSuccess(() -> message, false);
            return 1;
        }))).then(Commands.literal("isEnchantedBlockGetatable").then(Commands.argument("enabled", BoolArgumentType.bool()).executes(context -> {
            boolean preStatus = Config.isEnchantedBlockGetatable();
            boolean enabled = BoolArgumentType.getBool(context, "enabled");
            if (preStatus == enabled) {
                Component message = Component.literal("§a[isEnchantedBlockGetatable]§r").append(Component.translatable("command.random_enchant.config.unchanged"));
                RandomEnchant.LOGGER.info(message.getString());
                context.getSource().sendSuccess(() -> message, false);
                return 1;
            }
            Config.setIsEnchantedBlockGetatable(enabled);
            Component message = Component.literal("§a[isEnchantedBlockGetatable]§r").append(Component.translatable("command.random_enchant.config.changed")).append(enabled ? "§a[true]§r" : "§c[false]§r");
            RandomEnchant.LOGGER.info(message.getString());
            context.getSource().sendSuccess(() -> message, false);
            return 1;
        }))).then(Commands.literal("infinityPotion").then(Commands.argument("enabled", BoolArgumentType.bool()).executes(context -> {
            ...
            context.getSource().sendSuccess(() -> message, false);
            return 1;
        }))).then(Commands.literal("redirectTridentSetPointDistance").then(Commands.argument("value", IntegerArgumentType.integer(1, Integer.MAX_VALUE)).executes(context -> {
            ...
            context.getSource().sendSuccess(() -> message, false);
            return 1;
        }))).then(Commands.literal("flyEnchantmentLiftHeightPerTick").then(Commands.argument("value", DoubleArgumentType.doubleArg(0.00000001, 1)).executes(context -> {
